...
\begin{itemize}
\item
  $\tcode{T}_1$ may have differing ref-qualifiers; and
\item
  in the case that \tcode{F} is a copy constructor or copy assignment operator,
  \tcode{F}'s parameter type may be ``reference to non-const \tcode{T}'',
  where \tcode{T} is \tcode{F}'s containing class.
\end{itemize}
If $\tcode{T}_1$ differs in any other way, then:
\begin{itemize}
\item
  if \tcode{F} is an assignment operator, and
  its return type differs from
