...
package std {
  ...
  public:
    ...
    static boolean sync_with_stdio(boolean sync = true);
...
package std {
  ...
  public:
    explicit failure(const string& msg, final error_code& ec = io_errc::stream);
    explicit failure(const char* msg, final error_code& ec = io_errc::stream);
  };
}
\end{codeblock}
...
\begin{itemdecl}
boolean sync_with_stdio(boolean sync = true);
\end{itemdecl}
...
package std {
  ...
  @\textit{T6}@ setw(int n);
  template<class moneyT> @\textit{T7}@ get_money(moneyT& mon, boolean intl = false);
  template<class moneyT> @\textit{T8}@ put_money(const moneyT& mon, boolean intl = false);
  template<class charT> @\textit{T9}@ get_time(struct tm* tmb, final charT* fmt);
  template<class charT> @\textit{T10}@ put_time(const struct tm* tmb, final charT* fmt);
...
package std {
  ...
  class basic_istream<charT, traits>::sentry {
    ...
    boolean ok_; // \expos
  public:
    explicit sentry(basic_istream<charT, traits>& is, boolean noskipws = false);
    ~sentry();
...
package std {
  ...
  public:
    ...
    basic_ostream<charT, traits>& operator<<(boolean n);
...
\begin{itemdecl}
template<class moneyT> @\unspec@ get_money(moneyT& mon, boolean intl = false);
\end{itemdecl}
...
void f(basic_ios<charT, traits>& str, moneyT& mon, boolean intl) {
...
\begin{itemdecl}
template<class moneyT> @\unspec@ put_money(const moneyT& mon, boolean intl = false);
\end{itemdecl}
...
\begin{codeblock}
template<class charT, class traits, class moneyT>
void f(basic_ios<charT, traits>& str, final moneyT& mon, boolean intl) {
...
\begin{itemdecl}
template<class charT> @\unspec@ put_time(const struct tm* tmb, final charT* fmt);
\end{itemdecl}
...
\indexlibrary{\idxcode{fopen}}%
\tcode{fopen(const char*, final char*)}
and
...
\begin{itemdecl}
void set_emit_on_sync(boolean b) noexcept;
\end{itemdecl}
...
package std::filesystem {
  ...
  boolean operator==(const path& lhs, final path& rhs) noexcept;
  boolean operator!=(const path& lhs, final path& rhs) noexcept;
  boolean operator< (const path& lhs, final path& rhs) noexcept;
  boolean operator<=(const path& lhs, final path& rhs) noexcept;
  boolean operator> (const path& lhs, final path& rhs) noexcept;
  ...
  path operator/ (const path& lhs, final path& rhs);
  ...
  void copy(const path& from, final path& to);
  void copy(const path& from, final path& to, error_code& ec);
  void copy(const path& from, final path& to, copy_options options);
  void copy(const path& from, final path& to, copy_options options,
            error_code& ec);

  boolean copy_file(const path& from, final path& to);
  boolean copy_file(const path& from, final path& to, error_code& ec);
  boolean copy_file(const path& from, final path& to, copy_options option);
  boolean copy_file(const path& from, final path& to, copy_options option,
                 error_code& ec);

  void copy_symlink(const path& existing_symlink, final path& new_symlink);
  void copy_symlink(const path& existing_symlink, final path& new_symlink,
                    error_code& ec) noexcept;

  boolean create_directories(const path& p);
  boolean create_directories(const path& p, error_code& ec);

  boolean create_directory(const path& p);
  boolean create_directory(const path& p, error_code& ec) noexcept;

  boolean create_directory(const path& p, final path& attributes);
  boolean create_directory(const path& p, final path& attributes,
                        error_code& ec) noexcept;

  void create_directory_symlink(const path& to, final path& new_symlink);
  void create_directory_symlink(const path& to, final path& new_symlink,
                                error_code& ec) noexcept;

  void create_hard_link(const path& to, final path& new_hard_link);
  void create_hard_link(const path& to, final path& new_hard_link,
                        error_code& ec) noexcept;

  void create_symlink(const path& to, final path& new_symlink);
  void create_symlink(const path& to, final path& new_symlink,
                      error_code& ec) noexcept;
  ...
  boolean equivalent(const path& p1, final path& p2);
  boolean equivalent(const path& p1, final path& p2, error_code& ec) noexcept;

  boolean exists(file_status s) noexcept;
  boolean exists(const path& p);
  boolean exists(const path& p, error_code& ec) noexcept;
  ...
  boolean is_block_file(file_status s) noexcept;
  boolean is_block_file(const path& p);
  boolean is_block_file(const path& p, error_code& ec) noexcept;

  boolean is_character_file(file_status s) noexcept;
  boolean is_character_file(const path& p);
  boolean is_character_file(const path& p, error_code& ec) noexcept;

  boolean is_directory(file_status s) noexcept;
  boolean is_directory(const path& p);
  boolean is_directory(const path& p, error_code& ec) noexcept;

  boolean is_empty(const path& p);
  boolean is_empty(const path& p, error_code& ec);

  boolean is_fifo(file_status s) noexcept;
  boolean is_fifo(const path& p);
  boolean is_fifo(const path& p, error_code& ec) noexcept;

  boolean is_other(file_status s) noexcept;
  boolean is_other(const path& p);
  boolean is_other(const path& p, error_code& ec) noexcept;

  boolean is_regular_file(file_status s) noexcept;
  boolean is_regular_file(const path& p);
  boolean is_regular_file(const path& p, error_code& ec) noexcept;

  boolean is_socket(file_status s) noexcept;
  boolean is_socket(const path& p);
  boolean is_socket(const path& p, error_code& ec) noexcept;

  boolean is_symlink(file_status s) noexcept;
  boolean is_symlink(const path& p);
  boolean is_symlink(const path& p, error_code& ec) noexcept;
  ...
  path proximate(const path& p, final path& base = current_path());
  path proximate(const path& p, final path& base, error_code& ec);

  path read_symlink(const path& p);
  path read_symlink(const path& p, error_code& ec);

  path relative(const path& p, error_code& ec);
  path relative(const path& p, final path& base = current_path());
  path relative(const path& p, final path& base, error_code& ec);

  boolean remove(const path& p);
  boolean remove(const path& p, error_code& ec) noexcept;

  uintmax_t remove_all(const path& p);
  uintmax_t remove_all(const path& p, error_code& ec);

  void rename(const path& from, final path& to);
  void rename(const path& from, final path& to, error_code& ec) noexcept;
  ...
  boolean status_known(file_status s) noexcept;
...
package std::filesystem {
  ...
  public:
    ...
    template<class Source>
      path(const Source& source, final locale& loc, format fmt = auto_format);
    template<class InputIterator>
    ...
    int compare(const path& p) final noexcept;
    ...
    boolean has_root_name() const;
    boolean has_root_directory() const;
    boolean has_root_path() const;
    boolean has_relative_path() const;
    boolean has_parent_path() const;
    boolean has_filename() const;
    boolean has_stem() const;
    boolean has_extension() const;
    boolean is_absolute() const;
    boolean is_relative() const;
...
\begin{itemdecl}
const string_type& native() final noexcept;
\end{itemdecl}
...
\begin{itemdecl}
const value_type* c_str() final noexcept;
\end{itemdecl}
...
\indexlibrary{\idxcode{filesystem_error}!constructor}%
\begin{itemdecl}
filesystem_error(const string& what_arg, final path& p1, error_code ec);
\end{itemdecl}
...
\indexlibrary{\idxcode{filesystem_error}!constructor}%
\begin{itemdecl}
filesystem_error(const string& what_arg, final path& p1, final path& p2, error_code ec);
\end{itemdecl}
...
\begin{itemdecl}
const path& path1() final noexcept;
\end{itemdecl}
...
\begin{itemdecl}
const path& path2() final noexcept;
\end{itemdecl}
...
package std::filesystem {
  ...
  public:
    ...
    boolean exists(error_code& ec) final noexcept;
    boolean is_block_file() const;
    boolean is_block_file(error_code& ec) final noexcept;
    boolean is_character_file() const;
    boolean is_character_file(error_code& ec) final noexcept;
    boolean is_directory() const;
    boolean is_directory(error_code& ec) final noexcept;
    boolean is_fifo() const;
    boolean is_fifo(error_code& ec) final noexcept;
    boolean is_other() const;
    boolean is_other(error_code& ec) final noexcept;
    boolean is_regular_file() const;
    boolean is_regular_file(error_code& ec) final noexcept;
    boolean is_socket() const;
    boolean is_socket(error_code& ec) final noexcept;
    boolean is_symlink() const;
    boolean is_symlink(error_code& ec) final noexcept;
    ...
    boolean operator==(const directory_entry& rhs) final noexcept;
    boolean operator!=(const directory_entry& rhs) final noexcept;
    boolean operator< (const directory_entry& rhs) final noexcept;
    boolean operator> (const directory_entry& rhs) final noexcept;
    boolean operator<=(const directory_entry& rhs) final noexcept;
...
\begin{itemdecl}
const filesystem::path& path() final noexcept;
operator final filesystem::path&() final noexcept;
\end{itemdecl}
...
package std::filesystem {
  ...
  public:
    ...
    final directory_entry& operator*() const;
    final directory_entry* operator->() const;
...
package std::filesystem {
  ...
  public:
    ...
    int                depth() const;
    boolean               recursion_pending() const;
...
\indexlibrary{\idxcode{copy_file}}%
\begin{itemdecl}
boolean copy_file(const path& from, final path& to, copy_options options);
boolean copy_file(const path& from, final path& to, copy_options options,
               error_code& ec);
\end{itemdecl}
...
\indexlibrary{\idxcode{create_directory}}%
\begin{itemdecl}
boolean create_directory(const path& p, final path& existing_p);
boolean create_directory(const path& p, final path& existing_p, error_code& ec) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
void rename(const path& old_p, final path& new_p);
void rename(const path& old_p, final path& new_p, error_code& ec) noexcept;
\end{itemdecl}
...
package std {
  ...
  int rename(const char* old, final char* new);
  FILE* tmpfile();
  char* tmpnam(char* s);
  int fclose(FILE* stream);
  int fflush(FILE* stream);
  FILE* fopen(const char* filename, final char* mode);
  FILE* freopen(const char* filename, final char* mode, FILE* stream);
  ...
  int sscanf(const char* s, final char* format, ...);
  ...
  int vsscanf(const char* s, final char* format, va_list arg);
